...
allAustralia <- function(fun, bg=NULL, add.legend=TRUE, old=FALSE, regnames=NULL, maplim=c(-51, -8), ...){
  ...
  plot(0, type='n', xlim=xlim, ylim=maplim, asp=1/cos(mean(maplim)/180*pi), axes=FALSE, xlab='', ylab='', xaxs='i', yaxs='i')
  ## plot background
  ...
  if (!is.null(bg)){
    ...
    if (add.legend){
      addcolourbar <- function(){
        ...
        axis(1, at=seq(1.5, length(bg$col) - 0.5), labels=bg$lev[-c(1, length(bg$lev))], cex.axis=0.8*par('cex.axis'), line=-0.7, tick=FALSE)
        if (!is.null(bg$units)) axis(1, at=length(bg$col)+0.5, bg$units, tick=FALSE, hadj=0.2, cex.axis=0.8*par('cex.axis'), line=-0.7)
        if (!is.null(bg$title)) axis(3, at=0.5, bg$title, font=2, tick=FALSE, hadj=0, line=-0.7, cex.axis=0.8*par('cex.axis'))        
      }
